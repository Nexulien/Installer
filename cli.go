...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Nexulien")
	var updateFlag = flag.Bool("repair", false, "Repair Nexulien")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Nexulien")
	...
	if *versionFlag {
		fmt.Println("Nexulien Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Nexulien Installer' option to update, or run with --update-self")
			}
		}()
		...
		case "Quit":
			return
		case "Update Nexulien Installer":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest Nexulien files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Nexulien will likely not work.")
}
